...
import toolErrorRatesLong from "../../../public/data/hra/tool_error_rates_long.json";
import topErrorsByTool from "../../../public/data/hra/top_errors_by_tool.json";
import toolReturnRateData from "../../../public/data/hra/tool_return_rate.json";
import allToolErrorRates from "../../../public/data/hra/all_tool_error_rates.json";
import yearlyData from "../../../public/data/hra/tool_visits_by_year.json";
import totalData from "../../../public/data/hra/total_tool_visits.json";
import hourlyHeatmapData from "../../../public/data/hra/tool_hourly_heatmap.json";
import dowData from "../../../public/data/hra/traffic_by_dow.json";
import eventTypes from "../../../public/data/hra/event_types.json";
import errorClusters from "../../../public/data/hra/error_clusters.json";
import monthlyErrorData from "../../../public/data/hra/monthly_error_trend.json";
import errorBreakdown from "../../../public/data/hra/error_breakdown.json";
import errorRootCauseBreakdown from "../../../public/data/hra/error_root_cause_breakdown.json";
import externalEvents from "../../../public/data/hra/external_events.json";
import hraReleases from "../../../public/data/hra/hra_releases.json";
import publicationsData from "../../../public/data/hra/publications.json";
...
const clusterTotal = (errorClusters as { total_error_rows?: number }).total_error_rows ?? 0;
...
export default function ToolsPage() {
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* All-time error rate comparison */}
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-1 h-5 rounded-full bg-red-400" />
          <span className="text-base font-semibold text-zinc-800 dark:text-zinc-200">All-Time Error Rate by Tool</span>
          <span className="text-xs text-zinc-500 dark:text-zinc-600 ml-2">errors per 100 tool visits (cumulative)</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {(allToolErrorRates as { tool: string; visits: number; errors: number; error_rate: number }[]).map((t) => {
            const best = allToolErrorRates.reduce((min, d) => (d.error_rate < min.error_rate ? d : min), allToolErrorRates[0]);
            const worst = allToolErrorRates.reduce((max, d) => (d.error_rate > max.error_rate ? d : max), allToolErrorRates[0]);
            const isBest = t.tool === best.tool;
            const isWorst = t.tool === worst.tool;
            return (
              <div
                key={t.tool}
                className={`rounded-lg p-4 flex flex-col gap-1.5 border ${
                  isBest
                    ...
                    : "bg-zinc-100 dark:bg-zinc-800/40 border-zinc-200 dark:border-transparent"
                }`}
              >
                <span className={`text-xs font-semibold uppercase tracking-wider ${TOOL_CARD_COLORS[t.tool] ?? "text-zinc-400"}`}>
                  {t.tool}
                </span>
                <span className={`text-2xl font-bold tabular-nums ${isBest ? "text-emerald-500" : isWorst ? "text-red-400" : "text-zinc-800 dark:text-zinc-200"}`}>
                  {t.error_rate}%
                </span>
                <span className="text-[11px] text-zinc-500">
                  {t.errors.toLocaleString()} errors / {t.visits.toLocaleString()} visits
                </span>
                {isBest && <span className="text-[10px] font-medium text-emerald-500">Lowest error rate</span>}
                {isWorst && <span className="text-[10px] font-medium text-red-400">Highest error rate</span>}
              </div>
            );
          })}
        </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ErrorSourceChart errorBreakdown={errorBreakdown} />
          ...
          <div>
            ...
            <ErrorCauseChart errorClusters={errorClusters} />
            <p className="text-xs text-zinc-500 mt-2">
