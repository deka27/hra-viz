#!/usr/bin/env python3
"""Extract parquet field metadata into public/data/hra/parquet_field_dictionary.json."""
...
def _latest_parquet(directory: str) -> Path:
    import glob, os
    files = sorted(glob.glob(os.path.join(directory, "*.parquet")), key=os.path.getmtime, reverse=True)
    return Path(files[0]) if files else Path("data/hra/2026-04-06_hra-logs.parquet")

DEFAULT_PARQUET = _latest_parquet("data/hra")
DEFAULT_OUT = Path("public/data/hra/parquet_field_dictionary.json")
